...
```
The assay-tracking-network@0.0.2-deploy.28.bna is the core file needed by the Hyperledger Composer. It was made up with the files in the assay-tracking-network@0.0.2-deploy.28 directory. The lib folder contains a logic.js file, which defined the functions of the network. The model folder includes a assay-tracking-network@0.0.2-deploy.28.cto file, which defined the architecture and logic of the blockchain network. The queries.qry priovides the query service. The packages.jason provides the information of the dependence, and the permission.acl is for access control.
